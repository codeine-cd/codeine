...
function buildNodesByVersion() {
	...
	for (var i=0; i < nodesJson.length ; i++) {
		...
		if (success[nodesJson[i].version] === undefined) {
			success[nodesJson[i].version] = [];
		}
		if (fail[nodesJson[i].version] === undefined) {
			fail[nodesJson[i].version] = [];
		}
		if (nodesJson[i].success_monitors.length > 0){
			success[nodesJson[i].version].push(nodesJson[i]);
		} else {
