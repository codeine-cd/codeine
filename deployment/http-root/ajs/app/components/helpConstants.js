    ...
    angular.module('codeine')
        .constant('HelpConstants', {
            ...
            "command_patameter_validation_expression_Help" : "A regular expression that will validate the parameter in the UI.<br/><br/>" +
                "For example: <strong>\\S*</strong> - a word, <strong>\\d</strong> - one digit number, <strong>.*</strong> - not empty (like required). add ^ and $ to force the entire expression.",
            ...
            "nodeTagsHelp" : "Tags on nodes.<br/>Press a tag to filter only nodes with that tag (blue). You can select the logic operator between tags in the toogle button (OR, AND)",
            ...
            "commandTags" : "Will filter commands menu based on the tags of"
                + " the selected nodes.<br/>Only commands with nodes that apply to selected nodes tags will be displayed",
            "collectorsTags" : "The collector will run only on nodes that"
