...
{
	my $java_cmd =
"/usr/bin/nohup $java -Djava.path=$java -Ddebug=true -Dyami.conf=$conf_file -Drsync.user=$user -Dport=$client_port -cp $client_install/bin/yami.jar yami.YamiClientBootstrap > $start_log 2>&1 < /dev/null &";
