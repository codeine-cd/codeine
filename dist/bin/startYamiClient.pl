...
{
	my $java_cmd =
"/usr/bin/nohup $java -Djava.path=$java -Ddebug=true -Dyami.conf=$conf_file -Drsync.user=$user -Dport=$client_port -Drsync.source=$rsync_source -cp $client_install/bin/yami.jar yami.YamiClientBootstrap > $start_log 2>&1 < /dev/null &";
...
sub is_pid_port_match    # need to finish yshabi
{
	...
	foreach my $line (@out)
	{
		...
		return 1
		  if ( $v[1] =~ m/^$pid$/
			&& $line =~
			m/($client_port[ \t]+\(LISTEN\)$)|($hostname:$client_port(->))/ );
	}
	return 0;
}
