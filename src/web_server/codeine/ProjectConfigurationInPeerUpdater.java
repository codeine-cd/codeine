...
public class ProjectConfigurationInPeerUpdater {
    ...
    @Inject
    private Links links;
    private ThreadPoolExecutor threadPool = ThreadPoolUtils
        .newThreadPool(12, "ProjectConfigurationInPeerUpdater");
        ...
        for (PeerStatusJsonV2 e : allPeers) {
            try {
                ...
                String result = HttpUtils.doGET(
                    links.getPeerLink(e.address_port() + Constants.RELOAD_CONFIGURATION_CONTEXT),
                    null, HttpUtils.SHORT_READ_TIMEOUT_MILLI);
                log.debug("updated " + e.address_port() + " result " + result);
            } catch (Exception e1) {
                failedPeers.add(e);
                log.warn("fail to push conf " + e.address_port() + "  " + ExceptionUtils
                    .getRootCauseMessage(e1));
            }
        }
        log.info(
            "finished to send refresh request to " + allPeers.size() + " peers, failed to send to "
                + failedPeers.size() + " ; took " + s);
    }
    ...
    public void updatePeers(ProjectJson updatedProject, ProjectJson previousProject) {
        ...
        if (peers.size() > 1000) {
            log.info(
                "Skipping updating project " + updatedProject.name() + " as it has " + peers.size()
                    + " peers");
            return;
        }
    ...
    private Collection<? extends PeerStatusJsonV2> getPeersFromProject(ProjectJson project) {
        Collection<PeerStatusJsonV2> peers = Sets.newHashSet();
        if (null == project
            || project.node_discovery_startegy() != NodeDiscoveryStrategy.Configuration) {
            return peers;
        }
