...
public class CodeineWebServerBootstrap extends AbstractCodeineBootstrap {
    ...
    protected void execute() {
        ...
        new PeriodicExecuter(MonitorsStatistics.SLEEP_TIME,
            injector().getInstance(IMonitorStatistics.class)).runInThreadSleepFirst();
        new PeriodicExecuter(TimeUnit.SECONDS.toMillis(5),
            injector().getInstance(NotificationsFetchAndUpdateTask.class)).runInThreadSleepFirst();
        new PeriodicExecuter(TimeUnit.SECONDS.toMillis(5),
            injector().getInstance(CommandFileWriter.class)).runInThreadSleepFirst();
    }
    ...
    protected ServletContextHandler createServletContextHandler() {
        AuthenticationMethod a = injector().getInstance(GlobalConfigurationJsonStore.class).get()
    ...
    private ServletContextHandler createServletContextHandlerBasic() {
        ...
        context
            .addServlet(new ServletHolder(injector().getInstance(LogoutServlet.class)), "/logout");
    ...
    private ServletContextHandler createServletContextHandlerSpnego() {
        GlobalConfigurationJsonStore config = injector()
            .getInstance(GlobalConfigurationJsonStore.class);
        ServletContextHandler context = new ServletContextHandler(ServletContextHandler.SESSIONS
            | ServletContextHandler.SECURITY);
