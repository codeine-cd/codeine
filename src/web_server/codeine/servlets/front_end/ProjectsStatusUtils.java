...
public class ProjectsStatusUtils {
	...
	public static List<NodeTemplate> getVersionsNodes(String projectName, String versionName, ProjectJson project, NodeGetter nodesGetter, Links links) {
		...
		for (NodeWithMonitorsInfo nodeInfo : nodes) {
			...
			versionNodes.add(new NodeTemplate(nodeInfo.alias(), nodeInfo.name(), nodeInfo.peer().host_port(), nodeInfo.peer().peer_key(), failingMonitors,nodeInfo.peer().status() == PeerStatusString.On ? "success" : "disc", nodeInfo.version(), nodeInfo.tags()));
		}
		return versionNodes;
	}
