...
public class ProjectConfigurationApiServlet extends AbstractApiServlet {
    ...
    @Inject
    private ConfigurationManagerServer configurationManager;
    @Inject
    private UserPermissionsGetter permissionsManager;
    @Inject
    private AfterProjectModifyPlugin afterProjectModifyPlugin;
    ...
    protected void myGet(HttpServletRequest request, HttpServletResponse response) {
        writeResponseJson(response, configurationManager
            .getProjectForName(getParameter(request, Constants.UrlParameters.PROJECT_NAME)));
    }
    ...
    protected void myPut(HttpServletRequest request, HttpServletResponse resp) {
        ...
        log.info("Updating configuration of " + projectJson.name() + ", new configuration is "
            + projectJson);
        ...
        if (null != currentProject && !isAdministrator(request) &&
            (!MiscUtils.equals(currentProject.nodes_discovery_script(),
                projectJson.nodes_discovery_script()) || !MiscUtils
                .equals(currentProject.node_discovery_startegy(),
                    projectJson.node_discovery_startegy()))) {
            log.warn("user tried to change discovery script in " + projectJson.name() + " user "
                + getUser(request).user().username());
            throw new IllegalArgumentException("only admin can change discovery script");
        }
        if (null != currentProject && !isAdministrator(request) && !checkForRootPermissions(
            projectJson, currentProject)) {
            log.warn(
                "user tried to change command or collector in " + projectJson.name() + " user " +
    ...
    protected void myDelete(HttpServletRequest request, HttpServletResponse response) {
        ...
        ProjectJson projectToDelete = JsonUtils
            .cloneJson(configurationManager.getProjectForName(projectName), ProjectJson.class);
        ...
        afterProjectModifyPlugin
            .call(projectToDelete, StatusChange.remove, getUser(request).user().username());
    ...
    private boolean checkForRootPermissions(ProjectJson newProjectConf,
        ProjectJson currentProjectConf) {
        List<CollectorInfo> rootCollectors = Lists
            .newArrayList(Iterables.filter(newProjectConf.collectors(), rootCollectorPredicate()));
        List<CommandInfo> rootCommands = Lists
            .newArrayList(Iterables.filter(newProjectConf.commands(), rootCommandPredicate()));
        log.info(
            "updated configuration has " + rootCollectors.size() + " collectors and " + rootCommands
                .size() +
                " commands running has root");
        List<CollectorInfo> currentRootCollectors = Lists.newArrayList(
            Iterables.filter(currentProjectConf.collectors(), rootCollectorPredicate()));
        List<CommandInfo> currentRootCommands = Lists
            .newArrayList(Iterables.filter(currentProjectConf.commands(), rootCommandPredicate()));
        rootCollectors.removeAll(currentRootCollectors);
        if (rootCollectors.size() > 0) {
            log.info(
                "Found " + rootCollectors.size() + " collectors that changed to root permissions "
                    + rootCollectors);
            return false;
        }
        ...
        if (rootCommands.size() > 0) {
            log.info(
                "Found " + rootCommands.size() + " collectors that changed to root permissions "
                    + rootCommands);
            return false;
        }
        return true;
    ...
    private Predicate<CommandInfo> rootCommandPredicate() {
        return new Predicate<CommandInfo>() {
            ...
            public boolean apply(CommandInfo commandInfo) {
                return !StringUtils.isEmpty(commandInfo.cred()) && commandInfo.cred()
                    .equals("root");
            }
        };
    }
    ...
    private Predicate<CollectorInfo> rootCollectorPredicate() {
        return new Predicate<CollectorInfo>() {
            ...
            public boolean apply(CollectorInfo collectorInfo) {
                return !StringUtils.isEmpty(collectorInfo.cred()) && collectorInfo.cred()
                    .equals("root");
            }
        };
    }
